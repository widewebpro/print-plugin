 ...
 * @since     1.0.0
 */
class MarketingCampaignVariable
{
    // Public Methods
    // =========================================================================
    ...
    {
        $marketing = (new Query())->select("*")->from('{{%print_campaign_builder}}')->all();
    ...
    {
        $marketing = (new Query())->select("*")->from('{{%print_campaign_builder}}')->where(['id' => $id])->one();
    ...
    {
        $static = (new Query())->select("file")->from('{{%print_campaign_builder}}')->where(['id' => $id])->one();
